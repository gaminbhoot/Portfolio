...
export default function AiChatSidebar({ isOpen, onClose }) {
  ...
  const simulateBotResponse = (userText) => {
    ...
    if (lower.includes("project") || lower.includes("portfolio") || lower.includes("highlights") || lower.includes("build") || lower.includes("code")) {
      reply = "Jay has built several notable projects:\n\n1. **SysAware ML Optimizer**: A distributed dual-path diagnostic & runtime tuning suite for LLMs featuring a RandomForest inference estimator and LAN fleet telemetry (FastAPI, PyTorch & React TS).\n2. **OctaWipe**: A secure storage data sanitization tool compliant with DoD standards.\n3. **Abhisar**: A streaming LLM workspace powered by the Groq Inference Engine (React & Express).\n4. **Real-Time AI Motion Detection & Tracking**: An end-to-end computer vision pipeline utilizing YOLOv8 & DeepSORT.\n\nSelect **Projects.jsx** in the sidebar to explore detailed walkthroughs!";
